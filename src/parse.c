...
HrParameter_s hr_parse_parameter(strviu viu) {
    ...
    char *type_string = hr_parse_type(type);
...
HrFunction_s hr_parse_function(strviu info, strviu definition) {
    ...
    if(params.size >= HR_MAX_PARAMETERS) {
...
//    hr_ParsedTypeDeclaration res = {0};
//
//    viu = sv_strip(viu, ' ');
//    if(sv_empty(viu))
//        return res;
...
//    hr_ParsedStruct res = {0};
//
//    hr_ParsedInfo parsed_info = hr_parse_info_text(info);
//    strcpy(res.info, parsed_info.text);
//    hr_ParsedInfo_kill(&parsed_info);
//
//
//
//    return res;
